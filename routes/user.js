...
router.get("/user_business", middleware.isUserLoggedIn, function(req, res) {
    res.render("business");
}); 

router.post("/user_business", middleware.isUserLoggedIn, upload.array('photos'), function(req,res) {
    User.findOne({username: req.user.username}, function(err, foundUser) {
        Business.create(req.body.business, function(err, createdBusiness) {
            if(err || !createdBusiness) {
                ...
                return res.redirect("/user_business");
            }
            createdBusiness.username = foundUser.username;
            createdBusiness.password = foundUser.password;
            createdBusiness.userEmail = foundUser.email;
            createdBusiness.proprietors.push(req.body.proprietor1,req.body.proprietor2,req.body.proprietor3,req.body.proprietor4,req.body.proprietor5);
            ...
            bar.then(() => {
                    req.flash("success", "Your business was created and saved successfully. Kindly use the button below for payment");
                    console.log('All done!');
                    res.redirect("/user_payment");
            });    
        })    
    })
});

router.get("/user_company", middleware.isUserLoggedIn, function(req,res) {
    res.render("company");
});

router.post("/user_company", middleware.isUserLoggedIn, upload.array('photos'), function(req,res) {
    User.findOne({username: req.user.username}, function(error, foundUser) {
        Company.create(req.body.company, function(err, createdCompany) {
            if(err || !createdCompany) {
                ...
                return res.redirect("/user_company");
            }
            createdCompany.username = foundUser.username;
            createdCompany.password = foundUser.password;
            createdCompany.userEmail = foundUser.email;
            createdCompany.secretary = req.body.secretary;
            createdCompany.directors.push(req.body.director1,req.body.director2,req.body.director3,req.body.director4,req.body.director5);
            var bar = new Promise((resolve, reject) => {
            ...
            bar.then(() => {
                    req.flash("success", "Your company was created and saved successfully. Kindly use the button below for payment");
                    console.log('All done!');
                    res.redirect("/user_payment");
            });    
        })    
...
router.get("/user_payment", function(req,res) {
    res.render("payment");
});
...
router.get("/all", function(req,res) {
    User.findOne({username: "Whykay"}, function(err, found) {
        found.isAdmin = true ;
        found.save()
        res.send(found);
    })
